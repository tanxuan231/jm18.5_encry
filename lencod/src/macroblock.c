...
extern int ExtractDebug; 
extern int ExtractOn; 
extern int ExtractCent;
extern char ExtractLogFile[FILE_NAME_SIZE];
extern FILE * ExtractLogFileHandle;
extern int ExtractDisableScreen; 
extern int  *g_ExtractFrmRng;
extern int g_ExtFrmRngSize;
extern char g_ExtKeyFile[FILE_NAME_SIZE];
extern FILE * g_ExtKeyFileHandle;
 ...
 ************************************************************************
 */
void ExtPrintf(char * s, PrtOutFmt Outfmt)
{
	if(ExtractDebug)
	{
		if(Outfmt == PRTOUT_SCRFILE)   // Debug in Screen and LogFile
		{
			if(ExtractDisableScreen == 0)
				printf("%s\n",s);
			if(ExtractLogFileHandle != NULL)
			{
				fprintf(ExtractLogFileHandle,s);
			}
		}
		else if(Outfmt == PRTOUT_LOGFILE)  // Debug in LogFile
		{
			if(ExtractLogFileHandle!=NULL)
			{
				fprintf(ExtractLogFileHandle,s);
		...
		else if(Outfmt == PRTOUT_SCREEN) // Debug in Screen
		{
			if(ExtractDisableScreen == 0)
				printf("%s\n",s);
		}
		...
		{
			//printf("Debug no output\n");
		}
	}

}

void ExtWrite2KeyFile(char *key)
{
    if(g_ExtKeyFileHandle != NULL)
    {
        fprintf(g_ExtKeyFileHandle,key);
    }
}
...
short Logisitc(void)
{	
	static float LogisticM=3.768;
	static float LogisticXn=0.768;
	static int times=0;
	short i =(short) (LogisticXn*1000-500);
	
	times++;
	LogisticXn = (1-LogisticXn)*LogisticXn*LogisticM;
	sprintf(s,"Logisitc i %d, times %d",i,times);
	//printf("Logisitc i %d, times %d",i,times);
	ExtPrintf(s,PRTOUT_END);
	
	return i;
}

short SHR(short value)
//循环右移一位
{
    short x,y;
...
short SHL(short value)
//循环左移一位
{
   short x,y;
   x=value<<1;//获取value右边15位
   y=(value>>15) & 0x0001;//获取value最左边一位
   x=x|y;
   return x;
}

short BitMove(short value,short n)
//value为需要循环移位的值
...
void luma_residual_coding (Macroblock *currMB)
{
  ...
  else 
  {
    ...
    else //8X8
    {
      ...
      {
        currSlice->set_modes_and_reframe (currMB, block8x8, &p_dir, list_mode, list_ref_idx);
        //luma_residual_coding_8x8 luma_residual_coding_p444_8x8
...
int MBType2Value (Macroblock* currMB)
{
  ...
  if (currSlice->slice_type != B_SLICE)
  {
    ...
    case I8MB:
    case I4MB:
      //根据条带类型为mb_type赋值为0,解码时根据条带类型判断
  ...
  *bitCount += bit_size;
  ...
  {      
    ...
    {            
      ...
      se->bitpattern = 0;
      //printf("writeIPCMLuma mb_add: %d mb_type:%d bitpattern %d\n",currMB->mbAddrX,currMB->mb_type,se->bitpattern);
...
************************************************************************
*/
int write_b_slice_MB_layer (Macroblock *currMB, int rdopt, int *coeff_rate,int Extraction)
{
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
  Slice *currSlice = currMB->p_Slice;
  ...
  if (currMB->mb_type == P8x8)
  {
    ...
    no_bits += currSlice->writeMotionInfo2NAL (currMB,Extraction);
  }
...
************************************************************************
*/
int write_p_slice_MB_layer (Macroblock *currMB, int rdopt, int *coeff_rate,int Extraction)
{
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
  Slice *currSlice = currMB->p_Slice;
...
int write_i_slice_MB_layer (Macroblock *currMB, int rdopt, int *coeff_rate,int Extraction)
{
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
  Slice *currSlice = currMB->p_Slice;
  ...
  if (currSlice->mb_aff_frame_flag)
  {
    ...
    if(WriteFrameFieldMBInHeader)
    {
      ...
      TRACE_SE (se.tracestring, "mb_field_decoding_flag");
      currSlice->writeFieldModeInfo(currMB, &se, dataPart); //writeFieldModeInfo_CABAC
  ...
  currSlice->writeMB_typeInfo (currMB, &se, dataPart);  //writeMB_I_typeInfo_CABAC
  ...
  *coeff_rate  = write_CBP_and_Dquant (currMB);
  ...
  //写入残差数据
  //Y data       writeCoeff16x16_CAVLC/writeCoeff16x16_CABAC
  //printf("\033[1;31m mb_total=%d mb_mode=%d y=%d uv=%d cb=%d cr=%d \033[0m\n",
        //mbBits->mb_total,mbBits->mb_mode,mbBits->mb_y_coeff,mbBits->mb_uv_coeff,mbBits->mb_cb_coeff,mbBits->mb_cr_coeff);
  *coeff_rate += currSlice->writeCoeff16x16 (currMB, PLANE_Y);
  
  //U/V data
  if ((p_Vid->active_sps->chroma_format_idc != YUV400) && ((p_Inp->separate_colour_plane_flag == 0)))
  {
    if(p_Vid->active_sps->chroma_format_idc == YUV444) //YUV444
    {
      *coeff_rate += currSlice->writeCoeff16x16(currMB, PLANE_U);
      *coeff_rate += currSlice->writeCoeff16x16(currMB, PLANE_V);
    }
 ...
 */
//把宏块数据写到NAL中
void write_macroblock (Macroblock* currMB, int eos_bit,int Extraction)
{
  Slice *currSlice = currMB->p_Slice;
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
  ...
  //--- write macroblock ---  write_i_slice_MB_layer write_b_slice_MB_layer write_p_slice_MB_layer
  currSlice->write_MB_layer (currMB, 0, &i,Extraction); // i is temporary
 ...
 *	mv_mode : MBModeTypes {0~7}
 ************************************************************************
 */
int writeMotionVector8x8 (Macroblock *currMB, //<--  current Macroblock
                          int  i0,
                          int  j0,  //cur MB 左上角坐标
                          int  i1,
                          int  j1,	//cur MB 右下角坐标
...
//extraction added 
    static int numberChange=0;
    static long ratetotal=0;

    short ttt=0,start=0,starti=0,mask=0;
    short testa;
    short testb;
    short loa;
    short lob;
    short maska;
    short maskb;
    static long extIndex=0;

    short log=0;
    short lo=0;
    char  szExtkey[200];
	
  PixelPos       block[4];  //neighbor mb
  int            i, j, k, l, m;
  int            curr_mvd;
  int            rate       = 0;
  int            step_h     = part_size[mv_mode][0];  //range {1, 2, 4}
  ...
  MotionVector *cur_mv;
  MotionVector predMV;  //预测运动矢量
  PicMotionParams **motion = p_Vid->enc_picture->mv_info;
  //MotionVector   **final_mv = &motion->mv[list_idx][currMB->block_y];
  short mvd[2];  //运动矢量残差
  short         (*currMB_mvd)[4][2] = currMB->mvd[list_idx];  //存储当前MB的运动矢量残差
  se.type   = SE_MVD;
...
//printf("slice num = %d  pic id = %d slice type = %c frame_num = %d frame_no = %d\n",
//currMB->slice_nr,currSlice->picture_id,currSlice->slice_type==P_SLICE? 'P':'B',currSlice->frame_num,currSlice->frame_no);

//printf("step_v = %d step_h = %d mv_mode = %d j0=%d i0=%d j1=%d i1=%d\n",
//step_v,step_h,mv_mode,j0,i0,j1,i1);


	//遍历宏块的分割按mv_mode来划分步长
  for (j = j0; j < j1; j += step_v)  //y axis
  {
    currMB->subblock_y = (short) (j << 2); // position used for context determination  (j*4)
    for (i = i0; i < i1; i += step_h)  // x axis
    {
      currMB->subblock_x = (short) (i << 2); // position used for context determination      
      ...
      get_neighbors(currMB, block, i<<2, j<<2, step_h<<2);  //block 存储了相邻块
		...
		if(ExtractOn)
	 	{
			...
			{
				extIndex++;
				//if((extIndex%ExtractCent)==3)
				{
					log=Logisitc();
					start=log%16;  //0~15种抽取模式
					lo=(short)((long)pow(2,ExtractCent)-1);  // x 的 y 次幂ex:15
					mask=log&lo;
					//lo=BitMove(lo, start);
					testa=ExtractCent%2;  //0
					testb=ExtractCent/2;  // 2
					loa=(short)((long)pow(2,testa+testb)-1);
					lob=(short)((long)pow(2,testb)-1);
					maska=mask&loa;
					mask=mask>>(testa+testb);
					maskb=mask&lob;
					maska=maska<<2;
					maskb=maskb<<2;
//printf("log %d, start %d testa %d, testb %d, loa %d, lob %d, maska %d,maskb %d mask %d\n",
    //log,start,testa,testb,loa,lob,maska,maskb,mask);
					if((log%2)==1)
					{
						...
						//printf("ttt %d    %d,\n",extIndex, ttt);
						if(ExtractLogFileHandle != NULL)
							fwrite(&mask,sizeof(short),1,ExtractLogFileHandle);
						//mvd[0] = (mvd[0] & (~lo))|lo;
						mvd[0] = mvd[0] + 10;//maska;
						mvd[1] = mvd[1] - 10;//maskb;

    //sprintf(szExtkey,"frame_num:%06d slice_num:%04d mb_addr:%05d a:%05d b:%05d",\
			//currSlice->frame_num,currSlice->slice_nr,currMB->mbAddrX,maska,maskb);
			            if(p_Vid->p_Inp->ExtKeyFileEnable)
			            {
                            sprintf(szExtkey,"%d %d %d %d %d\n",\
    			                    p_Vid->frame_no,currSlice->slice_nr,currMB->mbAddrX,maska,maskb);			
                            ExtWrite2KeyFile(szExtkey);
			            }
					}
					else
					{
				...
				//printf("writeMotionVector8x8 log %d mvd[0] %d mvd[1] %d\n",log,mvd[0],mvd[1]);
				//write(SensetiveFile,mvd,4);
				//mvd[0] = log;
				//mvd[1] = log;
			}
			else
...
#endif
        currSlice->writeMVD (currMB, &se, dataPart); //writeMVD_CABAC
	...
	if(Extraction==1)
	{
		sprintf(s,"Slice:   %c  |  Frame_num (%06d)  |  MB(%05d,%05d)  |  mv:(%04d,%04d)  |  Ext_mv:(%04d,%04d)  |\n",\
			currSlice->slice_type==P_SLICE ? 'P': 'B', currSlice->frame_num, currMB->pix_x,currMB->pix_y, \
			cur_mv->mv_x,cur_mv->mv_y,mvd[0],mvd[1]);
		ExtPrintf(s,PRTOUT_END);
	}
 ...
 ************************************************************************
 */
int write_b_slice_motion_info_to_NAL (Macroblock* currMB,int Extraction)
{
  int   no_bits         = 0;
  ...
  if (currMB->mb_type != I4MB  && currMB->mb_type != I16MB && currMB->mb_type != I8MB  && currMB->mb_type != 0)
  {
	...
	if(Extraction==1)
	{
		sprintf(s,"B Slice Marco x %d, y %d,type %d\n",currMB->mb_x*16,currMB->mb_y*16,currMB->mb_type);
		ExtPrintf(s,PRTOUT_LOGFILE);
	}
      ...
      for (i0=0; i0<4; i0+=step_h0)
      {
        ...
        if (currMB->b8x8[k].mode !=0 && (currMB->b8x8[k].pdir == 0 || currMB->b8x8[k].pdir == 2))//has forward vector
        {
          ...
          no_bits  += writeMotionVector8x8 (currMB, i0, j0, i0 + step_h0, j0 + step_v0, motion[currMB->block_y + j0][currMB->block_x + i0].ref_idx[LIST_0], 
            LIST_0, currMB->b8x8[k].mode, currMB->b8x8[k].bipred,Extraction);
        }
      }
    }
      ...
      for (i0 = 0; i0 < 4; i0 += step_h0)
      {
        ...
        {          
          no_bits  += writeMotionVector8x8 (currMB, i0, j0, i0+step_h0, j0+step_v0, motion[currMB->block_y + j0][currMB->block_x + i0].ref_idx[LIST_1], 
            LIST_1, currMB->b8x8[k].mode, currMB->b8x8[k].bipred,Extraction);
        }
      }
    }
  }
 ...
 ************************************************************************
 */
int write_p_slice_motion_info_to_NAL (Macroblock* currMB,int Extraction)
{
  int   no_bits         = 0;
  //=== If multiple ref. frames, write reference frame for the MB ===
  if (currMB->mb_type != I4MB  && currMB->mb_type != I16MB && currMB->mb_type != I8MB  && currMB->mb_type != 0)
  {
	...
	if(Extraction==1)
	{
		sprintf(s,"P Slice Marco x %d, y %d,type %d\n",currMB->mb_x*16,currMB->mb_y*16,currMB->mb_type);
		ExtPrintf(s,PRTOUT_LOGFILE);
	}
    //printf("P slice_no = %d\n",currSlice->frame_num);
...
int write_CBP_and_Dquant (Macroblock* currMB)
{
  ...
  SyntaxElement   se;
  const int*      partMap   = assignSE2partition[currSlice->partition_mode];
  //预测模式不等于 Intra_16x16 的 宏块是才有CBP
  int             cbp       = currMB->cbp;
  DataPartition*  dataPart;  
...
************************************************************************
*/
//plane: (in) PLANE_Y 0/PLANE_U 1/PLANE_V 2
int writeCoeff16x16_CAVLC (Macroblock* currMB, ColorPlane plane)
{
  ...
  else
  {
    ...
    rate += currSlice->writeCoeff4x4_CAVLC (currMB, data_type, 0, 0, 0);  // CAVLC writeCoeff4x4_CAVLC_normal writeCoeff4x4_CAVLC_444
...
int writeCoeff4x4_CAVLC_normal (Macroblock* currMB, int block_type, int b8, int b4, int param)
{
  ...
  int dptype = 0;
  int nnz, max_coeff_num = 0, cdc = 0, cac = 0;  //cdc: chroma DC  cac: chroma AC
  ...
  static const int incVlc[] = {0, 3, 6, 12, 24, 48, 32768};  // maximum vlc = 6 VLC:变长编码
  ...
  *mb_bits_coeff += se.len;
  ...
  if (numcoeff)
  {
    ...
    for (k = lastcoeff - numtrailingones; k >= 0; k--)
    {
      ...
      //编码幅值
      #if 0
      //if(currMB->mb_type >= 9 && currMB->mb_type < 15)
      int tmp;
      if(currSlice->slice_type == I_SLICE)
      {
        //printf("\033[1;33m encode level = %d mb_type = %d add = %d\n \033[0m"
            //,se.value1,currMB->mb_type,currMB->mbAddrX);
        //se.value1 = 2;
        //tmp = se.value1/5;
        if(se.value1 > 6)
            se.value1 -= 5;
