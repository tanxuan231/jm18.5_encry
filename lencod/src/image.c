...
static void code_a_plane(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  int SliceGroup = 0;  //SliceGroup ID
  ...
  FmoInit(p_Vid, p_Vid->active_pps, p_Vid->active_sps);   //灵活块映射多片组模式
  FmoStartPicture (p_Vid);           //! picture level initialization of FMO

  CalculateQuant4x4Param (p_Vid);   //计算量化值
  CalculateOffset4x4Param(p_Vid);   //量化偏移
  ...
  {
    //printf("++++++++code_a_plane: cur slice num = %d \n",p_Vid->frame_no);
    printf("\033[1;31m tmp_test===============%s frame num = %d current_slice_nr = %d NumberOfCodedMBs = %d ======= \033[0m \n",
    __FUNCTION__,p_Vid->frame_no,p_Vid->current_slice_nr,NumberOfCodedMBs);
...
static int read_input_data(VideoParameters *p_Vid)
{
  ...
  int file_read = 0;
    ...
    if(p_Inp->num_of_views==2 && p_Vid->view_id == 1)
    {
      //p_Vid->imgData0.frm_data存放了原始帧的Y/U/V data 对应H264visa中的Org YUV
...
#if (MVC_EXTENSION_ENABLE)
  if(p_Vid->view_id != 1 || !p_Vid->sec_view_force_fld) //view_id 0~1
  {
#endif
    ...
    {
      frame_picture (p_Vid, p_Vid->frame_pic[0], &p_Vid->imgData, 0);  //编码图像p_Vid->frame_pic[0]传入要编码的一帧
...
int encode_one_frame (VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  if( (p_Inp->separate_colour_plane_flag != 0) )  //4:4:4 处理
  {
  ...
  */
  ...
  {
    if ( !read_input_data (p_Vid) ) //读入原始Y/U/V数据到p_Vid->imgData0.frm_data
    {
      return 0;
    }
  }

  process_image(p_Vid, p_Inp); //将待编码帧数据(Y/U/V)拷贝到p_Vid->imgData中
  ...
  if (p_Inp->PicInterlace == FIELD_CODING)  
    perform_encode_field(p_Vid);   //场编码
  else
...
static void init_frame (VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  if( (p_Inp->separate_colour_plane_flag != 0) )  //444
  {
  ...
  {
    for(i = 0; i < ((int) (p_Vid->FrameSizeInMbs)); i++)
      p_Vid->mb_data[i].slice_nr = -1;  //将所有宏块对应的slice_nr初始化为-1
  }
  ...
  init_dec_ref_pic_marking_buffer(p_Vid); //初始化解码参考图像标志
...
static void put_buffer_frame(VideoParameters *p_Vid)
{
  p_Vid->pCurImg    = p_Vid->imgData.frm_data[0]; //此时Y Data都为0
  p_Vid->pImgOrg[0] = p_Vid->imgData.frm_data[0];    
  
  if (p_Vid->yuv_format != YUV400)
  {
    p_Vid->pImgOrg[1] = p_Vid->imgData.frm_data[1]; //此时U Data都为128
    p_Vid->pImgOrg[2] = p_Vid->imgData.frm_data[2]; //此时V Data都为128 初值
  }
