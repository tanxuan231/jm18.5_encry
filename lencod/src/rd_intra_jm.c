 ...
 *************************************************************************************
 */
//4X4帧内预测,残差计算,参差DCT,Zig排序,量化,RL编码,IDCT,反量化
//存储重建象素,便于以后使用
int mode_decision_for_I4x4_blocks_JM_High (Macroblock *currMB, int  b8,  int  b4,  int  lambda,  distblk*  min_cost)
{
  ...
  //当左边块(A)或上边块(B)不可用时,取DC预测模式
  //否则取A(up)/B(left)预测模式中较小值
  //标准8.3.1.1:predIntra4x4PredMode = Min( intraMxMPredModeA, intraMxMPredModeB )  
  ...
  *min_cost = DISTBLK_MAX;
  ...
  // set intra prediction values for 4x4 intra prediction  set_intrapred_4x4/set_intrapred_4x4_mbaff
  currSlice->set_intrapred_4x4(currMB, PLANE_Y, pic_pix_x, pic_pix_y, &left_available, &up_available, &all_available);  

  //===== LOOP OVER ALL 4x4 INTRA PREDICTION MODES ===== 循环所以4X4帧内预测模式(9种)
...
#endif      
      //得到4X4块在模式ipmode下的rd_cost值rdcost_for_4x4_intra_blocks/rdcost_for_4x4_intra_blocks_444
...
int Mode_Decision_for_IntraSubMBlocks(Macroblock *currMB, int b8, int lambda, distblk *cost, int non_zero[3])
{
  ...
  *cost = weighted_cost(lambda, 6);        //6 * lambda;
  for (b4=0; b4<4; b4++)
  { 
    //4X4帧内预测,残差计算,参差DCT,Zig排序,量化,RL编码,IDCT,反量化,存储重建象素,便于以后使用
    //mode_decision_for_I4x4_blocks_JM_Low444 : mode_decision_for_I4x4_blocks_JM_Low
    //mode_decision_for_I4x4_blocks_JM_High444 : mode_decision_for_I4x4_blocks_JM_High;
...
int mode_decision_for_I16x16_MB_RDO (Macroblock* currMB, int lambda)
{
  ...
  //设置currMB->intra16x16_pred[pl]的预测像素值,得到周边宏块的可用性
  currSlice->set_intrapred_16x16(currMB, PLANE_Y, &left_avail, &up_avail, &left_up_avail);  //set_intrapred_16x16/set_intrapred_16x16_mbaff

  //VERT_PRED_16   = 0,HOR_PRED_16    = 1,DC_PRED_16     = 2,PLANE_16       = 3
  for (k = 0;k < 4; k++)
  {
    //判断是否满足对应的预测模式
    ...
    //将currMB->intra16x16_pred[pl]的预测像素值赋给currMB->p_Slice->mpr_16x16[pl]
    get_intrapred_16x16(currMB, PLANE_Y, k, left_avail, up_avail);

    currMB->i16mode = (char) k; 
    //residual_transform_quant_luma_16x16 / residual_transform_quant_luma_16x16_ls
    ...
    {
      //store_coding_state_cabac /store_coding_state_cavlc
      ...
      //CAVLC:writeUVLC_CAVLC |CABAC:writeMB_I_typeInfo_CABAC/writeMB_P_typeInfo_CABAC/writeMB_B_typeInfo_CABAC 
      currSlice->writeMB_typeInfo (currMB, &se, dataPart);

      rate = se.len;
      if (distortionY + weighted_cost(lambda, rate) < min_rdcost)
      {
        //writeCoeff16x16_CABAC writeCoeff16x16_CAVLC
