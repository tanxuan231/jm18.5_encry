 ...
 *	基于Lagrangian优化算法的率失真优化技术(RDO)
 *	实现对视频编码的控制，在{mode,MV,REF,Direction}
 *	中做出最佳选择
 *
 * \author
 *    - Heiko Schwarz
 ...
 *************************************************************************************
 * \brief
 *    R-D Cost for an 4x4 Intra block  返回rd_cost值
 *************************************************************************************
 */
static distblk rdcost_for_4x4_intra_blocks (Macroblock *currMB,
                                       ...
                                       int         b8,
                                       int         b4,
                                       int         ipmode, //DC_PRED ...帧内4X4预测模式9种
                                       int         lambda,
                                       ...
                                       distblk       min_rdcost)
{
  ...
  *nonzero = currMB->residual_transform_quant_luma_4x4 (currMB, PLANE_Y, block_x, block_y, &dummy, 1);
  ...
  //算出重建块和原始图像的差值平方 和Sum of Squared Error(SSE)称为distortion
  ...
  //--- encode and update rate ---算出编码该宏块需要的比特数,称为rate
                             ...
                             distblk  min_rdcost)     
{
  ...
  if (!direct)
  {
    ...
    if (pdir==0 || pdir==2)
    {
        //printf("1rdcost_for_8x8blocks slice_no = %d\n",currSlice->frame_num);
      rate  += writeMotionVector8x8 (currMB, i0, j0, i0 + 2, j0 + 2, l0_ref, LIST_0, mode, currMB->b8x8[block].bipred,0);
    }
    if (pdir==1 || pdir==2)
    {
      //printf("2rdcost_for_8x8blocks slice_no = %d\n",currSlice->frame_num);  
      rate  += writeMotionVector8x8 (currMB, i0, j0, i0 + 2, j0 + 2, l1_ref, LIST_1, mode, currMB->b8x8[block].bipred,0);
    }
  }
...
int RDCost_for_macroblocks (Macroblock  *currMB,   // <-- Current Macroblock to code
                            int   lambda,     // <-- lagrange multiplier 拉格朗日乘子
                            short    mode)    // <-- mode (0-COPY/DIRECT, 1-16x16, 2-16x8, 3-8x16, 4-8x8(+), 5-Intra4x4, 6-Intra16x16)                            
{
  ...
  //=====  set_modes_and_refs_for_blocks_i_slice/set_modes_and_refs_for_blocks_p_slice/set_modes_and_refs_for_blocks_b_slice
  currSlice->set_modes_and_refs_for_blocks (currMB, mode);
  //=====
  //=====  Set Motion Vectors
  //=====  SetMotionVectorsMBISlice(empty function)  SetMotionVectorsMBPSlice SetMotionVectorsMBBSlice
  currSlice->set_motion_vectors_mb (currMB);  //an empty function 

  //=====
  //=====  Get coefficients, reconstruction values, CBP etc
  //=====
  //对mode取不同值选择不同的模式决策函数
//PSKIP        =  0,
//BSKIP_DIRECT =  0,
//P16x16       =  1,
//P16x8        =  2,
...
//SMB4x4       =  7,
  ...
  if (mode < P8x8)
  {
    //luma_residual_coding luma_residual_coding_sp luma_residual_coding_p444
  ...
  { //mode_decision_for_I16x16_MB/mode_decision_for_I16x16_MB_444/mode_decision_for_I16x16_MB_RDO
  ...
  if (use_of_cc)
  {
    if (currMB->mb_type!=0 || (currSlice->slice_type == B_SLICE && currMB->cbp!=0))
    {
      ...
      rate = currSlice->write_MB_layer (currMB, 1, &coeff_rate,0);
