                          ...
                          short *inter_skip)
{
  ...
  currSlice->set_modes_and_refs_for_blocks (currMB, (short) mode); //set_modes_and_refs_for_blocks_i_slice
  ...
  if ((p_Inp->FastCrIntraDecision ) || (currMB->c_ipred_mode == DC_PRED_8 || (is_intra(currMB) )))
  {
    ...
    //bslice skip 
    //cbp: 高4位为色度,低4位为亮度(currMB->cbp&15) != 15表示非全部亮度需要传送
    ...
    //modes 0 and 1 of a B frame BSKIP_DIRECT 
