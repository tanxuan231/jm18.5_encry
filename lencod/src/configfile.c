...
void Configure (VideoParameters *p_Vid, InputParameters *p_Inp, int ac, char *av[])
{
  ...
  content = GetConfigFileContent (filename);  //read the file content to the content
  if (NULL==content)
    error (errortext, 300);
...
static void PatchInp (VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  //参考bit_depth_luma_minus8 
  if (p_Inp->src_BitDepthRescale)
  {
    //亮度量化参数范围的取值偏移 QpBdOffsetY 
    bitdepth_qp_scale [0] = 6*(p_Inp->output.bit_depth[0] - 8);
    bitdepth_qp_scale [1] = 6*(p_Inp->output.bit_depth[1] - 8); //u
    bitdepth_qp_scale [2] = 6*(p_Inp->output.bit_depth[2] - 8); //v 
  }
  else
  {
