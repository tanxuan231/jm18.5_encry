...
void set_chroma_pred_mode(Macroblock *currMB, RD_PARAMS enc_mb, int *mb_available, char chroma_pred_mode_range[2])
{
  ...
  if ((p_Vid->yuv_format != YUV400) && (p_Inp->separate_colour_plane_flag == 0))
  {
    ...
    //上/左/上左宏块    intra_chroma_prediction/intra_chroma_prediction_mbaff
    //帧内色度预测|色度的预测值保存在currMB->currSlice->mpr_16x16[1/2:u/v]中
    ...
    {             
      ...
      currSlice->intra_chroma_RD_decision(currMB, &enc_mb); //intra_chroma_RD_decision  intra_chroma_RD_decision_mbaff
