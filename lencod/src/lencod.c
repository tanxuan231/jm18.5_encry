...
int ExtractOn; 
int ExtractCent;
int ExtractDisableScreen;
char ExtractLogFile[FILE_NAME_SIZE];
FILE * ExtractLogFileHandle;
int  *g_ExtractFrmRng;  //全局数组存放需要提取的帧的序号
int  g_ExtFrmRngSize;   //g_ExtractFrmRng数组的大小
char g_ExtKeyFile[FILE_NAME_SIZE];
FILE * g_ExtKeyFileHandle;
 ...
 ***********************************************************************
 */
static void ext_prase_frm_rng(char *ss,int *top_half,int *btn_half)
{
    char *p = strtok(ss,"~");
    if(p)
    {
 ...
 ***********************************************************************
 * \brief
 *    解析需要提取的帧范围并保存到全局g_ExtractFrmRng数组中
 *  return: 返回全局g_ExtractFrmRng数组的长度
 ***********************************************************************
 */
int ext_save_frm_rng(char *ss)
{
    int top_half,btn_half;
    ext_prase_frm_rng(ss,&top_half,&btn_half);

    static int len,total_len;
    len = btn_half - top_half +1;
    total_len += len;

    if((g_ExtractFrmRng = (int *)realloc(g_ExtractFrmRng, sizeof(int)*total_len)) == NULL)
    {
        printf("realloc error\n");
        return;
    }
    
    int *tmp = g_ExtractFrmRng + (total_len - len);
    int i;
    for(i=0; i<len; i++,tmp++)
    {
        *tmp = top_half ++;
...
int main(int argc, char **argv)
{
  ...
  Configure (p_Enc->p_Vid, p_Enc->p_Inp, argc, argv);
	fprintf(stdout," ExtractionOn:		%d           ExtractionDebug		: %d \n",p_Enc->p_Inp->ExtractionOn,p_Enc->p_Inp->ExtractionPrint);
	fprintf(stdout," ExtractionCent:	%d           ExtractionLog		: %s \n",p_Enc->p_Inp->ExtractionCent,p_Enc->p_Inp->ExtractionLogFile);
    
	ExtractCent=p_Enc->p_Inp->ExtractionCent;
	ExtractDebug=p_Enc->p_Inp->ExtractionPrint;
	ExtractOn=p_Enc->p_Inp->ExtractionOn;
	ExtractDisableScreen=p_Enc->p_Inp->ExtractionDisableScreen;
	strcpy(ExtractLogFile,p_Enc->p_Inp->ExtractionLogFile);
    strcpy(g_ExtKeyFile,p_Enc->p_Inp->ExtKeyFile);

    if((g_ExtractFrmRng = (int *)malloc(sizeof(int))) == NULL)
    {
        printf("\n!!!!!!!!!!!!!g_ExtractFrmRng malloc fail\n\n");
    }
    g_ExtFrmRngSize = ext_save_frm_rng(p_Enc->p_Inp->ExtractFrmRng);
    
#if 1
    int i = 0;
    int *tmp = g_ExtractFrmRng;
    for(;i<g_ExtFrmRngSize;i++)
    {
        //printf("@@@@@@@@@@: %d \n",*tmp++);
    }
#endif

	if ((ExtractLogFileHandle = fopen(ExtractLogFile, "wb")) == NULL)    // append new statistic at the end
	{
		printf("\n!!!!!!!!!!!!!Log File Open Fail!!!!!!!!!!!!!\n\n");
	}
	else
    ...
    if(access(g_ExtKeyFile,0) == 0 && remove(g_ExtKeyFile) == 0)
    {
        printf("Key File Remove Success!\n");
    }
    if ((g_ExtKeyFileHandle = fopen(g_ExtKeyFile, "wb")) == NULL)
	{
		printf("\n!!!!!!!!!!!!!Key File Open Fail!!!!!!!!!!!!!\n\n");
	}
	else
	{
		printf("\n+++++++++++++Key File Open Success+++++++++++++\n\n");
	}
	/*
	fprintf(ExtractLogFileHandle,"===============================================================================\n");
	fprintf(ExtractLogFileHandle,"Decord File: %s  \nYUV File: %s  \nExtractionOn: %d,  ExtractionCent: %d\n",\
		p_Enc->p_Inp->input_file1.fname,p_Enc->p_Inp->outfile,p_Enc->p_Inp->ExtractionOn,p_Enc->p_Inp->ExtractionCent);
	now = time(NULL);
	strftime(buf, 24, "%Y-%m-%d %H:%M:%S", localtime(&now));
	fprintf(ExtractLogFileHandle,"Start From : %s \n",buf);
	fprintf(ExtractLogFileHandle,"Slice:Type  |  Slice (xxxxxx)  |  MB(xxxxx,xxxxx)  |  mv:(xxxx,xxxx)  |  Ext:(xxxx,xxxx)  |\n");
	*/
  ...
  free_encoder(p_Enc);
	...
	/*
	fprintf(ExtractLogFileHandle,"===============================================================================\n");
	fprintf(ExtractLogFileHandle,"End AT : %s \n",buf);
	*/
	fclose(ExtractLogFileHandle);
  return 0;
}
...
static void init_encoder(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  //parse log2_max_frame_num_minus4 -> fram_num:标识所属图像的解码顺序
  ...
  {
    if ((p_Inp->output.height[0] & 0x01) != 0)  //p_Inp->output.height[0]最低位为1
    {
      error ("even number of lines required for interlaced coding", 500);
    }

    if ((p_Inp->output.height[0] & 0x1F) != 0)  //p_Inp->output.height[0]最低5位有任何一位为1
    {
  ...
  //init dpb storage formage; dpb:decoded picture buffer
...
static void encode_sequence(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  {
    frames_to_code = p_Inp->no_frames << 1;  //* 2 因为views == 2
...
static int init_global_buffers(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  for (j = 0; j < p_Vid->frm_iter; j++)
    p_Vid->frame_pic[j] = malloc_picture(); //对frame_pic分配内存
