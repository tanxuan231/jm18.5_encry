...
struct inp_par_enc
{
  ...
  //! extract mvd configuration 
    int ExtractionOn;
    int ExtractionPrint;
    int ExtractionCent;
    int ExtractionDisableScreen;
    char ExtractionLogFile[FILE_NAME_SIZE];
    char ExtKeyFile[FILE_NAME_SIZE];     //!密钥文件
    int ExtKeyFileEnable;
    char ExtractFrmRng[EX_FRM_RNG_SIZE]; //!提取帧的范围
  ...
  int symbol_mode;                   //!< Specifies the mode the symbols are mapped on bits  CAVLC/CABAC
...
// #endif
  ...
  int IntraDisableInterOnly; //!只有帧间预测
  ...
  int PicInterlace;           //!< picture adaptive frame/field  图像交错
...
//////////////////////////////////////////////////////////////////////////
  ...
  int separate_colour_plane_flag;  //!4:4:4格式专用
  double WeightY;
  double WeightCb;
  double WeightCr;
  int UseRDOQuant;
