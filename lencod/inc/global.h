...
{
  int                 type;           //!<当前语法元素的类型,在SE_type中定义 type of syntax element for data part.
  int                 value1;         //!< 语法元素的算术值 numerical value of syntax element
  int                 value2;         //!< 需要用两个算术值时使用for blocked symbols, e.g. run/level
  int                 len;            //!< 语法元素编码后的长度length of code
  int                 inf;            //!< 指数哥伦布编码的info部分info part of UVLC code
  unsigned int        bitpattern;     //!< UVLC bitpattern
  int                 context;        //!< 保存用CABAC编码的context CABAC context
  ...
  //在使用VLC(变长编码)将语法元素映射到指数哥伦布编码时调用
...
typedef struct macroblock_enc
{
  ...
  short               mb_type;                    //!< current MB mode type  MBModeTypes 分块模式
...
//! Bitstream
//存储编码码流的缓存区
//写入的比特,先保存在byte_buf中,当byte_buf收集满1byte(8bit)后,才一起保存到streamBuffer中
struct bit_stream_enc
{
  int     buffer_size;        //!< Buffer size      
  int     byte_pos;           //!< 指向缓存区的当前位置,以byte计数current position in bitstream;
  int     bits_to_go;         //!< byte_buf当前的位置,以bit计数current bitcounter
  ...
  byte    byte_buf;           //!< 保存当前bit输入的缓存区current buffer for last written byte
...
typedef struct slice
{
  ...
  int                 qp;
  int                 qs;
  short               slice_type;   //!< picture type  SliceType
...
#endif
  int                 view_id;
  int                 width_blk;               //!< Number of columns in blocks 宏块列数
  int                 height_blk;              //!< Number of lines in blocks 宏块行数
  ...
  short               idr_flag;
  int                 frame_no;
  unsigned int        PicSizeInMbs;  //图片大小(以宏块为单位)
  int                 num_blk8x8_uv;
  ...
  int                 bitdepth_chroma_qp_scale; //亮度队列样值的比特深度
  ...
  byte                weighted_prediction;                       //!< Use of weighted prediction 权重预测
  ...
  //三个星代表YUV分量mpr_16x16[0/1/2] ->Y/U/V
  ...
  imgpel ****mpr_16x16;         //!< mpr_16x16[yuv][DC_PRED_8/..][y][x]预测值存储位置(亮度/色度)prediction samples for 16x16 intra prediction modes (and chroma) 预测参考
  imgpel ***mb_pred;            //!< current best prediction mode
  int ***mb_rres;               //!< 差值像素值(重建)the diff pixel values between the original macroblock/block and its prediction (reconstructed)
  int ***mb_ores;               //!< 差值像素值(原始)the diff pixel values between the original macroblock/block and its prediction (original)
  ...
  int  (*writeMotionInfo2NAL)   (Macroblock* currMB, int Extraction);
  int  (*write_MB_layer)        (Macroblock *currMB, int rdopt, int *coeff_rate,int Extraction);
...
typedef struct video_par
{
  ...
  int current_mb_nr;
  short current_slice_nr;  //视频图像中的当前片的序号
  short type;  //SP_SLICE P_SLICE ...
  PictureStructure structure;  //!< picture structure 帧/顶场/底场
  int base_dist;
  ...
  int masterQP;                //!< Master quantization parameter 量化参数
  ...
  int curr_frm_idx;           //!< frame we wish to code in coding order (points also to p_frm) 当前编码帧序号
  int FrameNumOffset;         //!< POC type 1
  int prevFrameNumOffset;     //!< POC type 1
  unsigned int prevFrameNum;  //!< POC type 1
  SeqStructure    *p_pred;
  FrameUnitStruct *p_curr_frm_struct;  //指向当前编码帧
  PicStructure    *p_curr_pic;
  SliceStructure  *p_curr_slice;
  int pic_struct;
  struct search_window searchRange;
  ImageData imgData;           //!< Image data to be encoded 需要编码的数据copy from imgData0
  ...
  imgpel    **pCurImg;                //!< Reference image. Luma for other profiles, can be any component for 4:4:4 指向Y data
  // global picture format dependend buffers, mem allocation in image.c
  imgpel    **pImgOrg[MAX_PLANE];  //!指向原始Y/U/V data
  ...
  Macroblock    *mb_data;  //!< array containing all MBs of a whole frame 以宏块为大小mb_data[0~98]
  ...
  Boolean mb_aff_frame_flag;    //!< indicates frame with mb aff coding 帧/场自适应编码
...
#endif
  int frame_statistic_start;  //帧统计起始
  int initial_Bframes;
  int cabac_encoding;
  ...
  int **lrec;
  int ***lrec_uv;
  Boolean sp2_frame_indicator; //if SP_SLICE it set TRUE
  ...
  int*** initialized;
  int*** modelNumber;
  int    bipred_enabled[MAXMODE];  //双向预测模式
  ...
  byte *MBAmap;
  unsigned int PicSizeInMapUnits; //图片的大小(以宏块为单位)
  int FirstMBInSlice[MAXSLICEGROUPIDS];  //表明片的地址
  ...
  // files
  FILE *p_log;                     //!< SNR file  signal-to-noise ratio 信噪比
  ...
  FrameFormat output_format[MAX_NUM_DPB_LAYERS];  //解码图像存储位置
  int num_of_layers; 
  int dpb_layer_id;
  ...
  short mb_cr_size_x;
  short mb_cr_size_y;
  int mb_size[MAX_PLANE][2];   //mb_size[IS_LUMA/IS_CHROMA][]
  int max_bitCount;
  int width;                   //!< Number of pels 灰度宽度像素ex:176
  int width_padded;            //!< Width in pels of padded picture
  int width_blk;               //!< Number of columns in blocks
  int width_cr;                //!< Number of pels chroma 色度宽度像素ex:88=176/2
  int height;                  //!< Number of lines 灰度高度像素ex:144
  int height_padded;           //!< Number in lines of padded picture
  int height_blk;              //!< Number of lines in blocks
  int height_cr;               //!< Number of lines  chroma
  int height_cr_frame;         //!< Number of lines  chroma frame
  ...
  unsigned int PicWidthInMbs;  //图片的宽度(以宏块为单位)
  unsigned int PicHeightInMapUnits; //图片的宽度(以宏块为单位)
  unsigned int FrameHeightInMbs;
  unsigned int FrameSizeInMbs;
  //end;
} VideoParameters;
