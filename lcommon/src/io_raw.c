...
static inline int ReadData (int vfile,  FrameFormat *source, unsigned char *buf)
{
  ...
  int i, j;
    ...
    //处理Y data
  for (i = 0; i < source->height[0]; i++) //按行读取，一次读一个宽度176*144 读9次每次176个数据
  {
...
int ReadFrameConcatenated (InputParameters *p_Inp, VideoDataFile *input_file, int FrameNoInFile, int HeaderSize, FrameFormat *source, unsigned char *buf)
{
  ...
  // Let us seek directly to the current frame  指向文件头后增加offset(起始帧+帧号)
  ...
  // Now read it.
  if ((source->pic_unit_size_on_disk & 0x07) == 0)  //pic_unit_size_on_disk能被8整除大小以字节为单位
  {
    if (is_v210 == TRUE)
    {
