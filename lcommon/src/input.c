                    ...
                    )
{
  ...
  else
  {
    ...
    if (size_x == o_size_x && size_y == o_size_y)
    {
      for (j=0; j < o_size_y; j++) //按行
      {
        j_pos = j * size_x;  //0*144;1*144;2*144....每行行首
        for (i=0; i < o_size_x; i++)
        {
          ui16=0;             
          //行首+一行的偏移(i + j_pos) * symbol_size_in_bytes表示字节数
          memcpy(&(ui16), buf + ((i + j_pos) * symbol_size_in_bytes), symbol_size_in_bytes);
          imgX[j][i]= (imgpel) ui16;  //imgX[j][i]里存放了177*144的数据
        }
      }    
    }
    else
 ...
 * \param source  (input par)
 *    source file (on disk) information 
 * \param output  (input par)
 *    output file (for encoding) information
 * \param pImage
 *    Image planes 存储了pImage[0]:Y data pImage[1]/[2]:U/V data
 ************************************************************************
 */
int read_one_frame (VideoParameters *p_Vid, VideoDataFile *input_file, int FrameNoInFile, int HeaderSize, FrameFormat *source, FrameFormat *output, imgpel **pImage[3])
{
  ...
  int file_read = 0;
  unsigned int symbol_size_in_bytes = source->pic_unit_size_shift3;  //一个像素用多少位来表示
  ...
  if (input_file->is_concatenated == 0) //single input
  {    
    if (input_file->vdtype == VIDEO_TIFF)  //file format tiff pic format
    {
  ...
  // De-interleave input source 是否是隔行扫描
  ...
  //处理Y Data数据
  //回调buf2img_basic |pImage[0]存放了原始未编码的Y data的数据
  if(rgb_input)
 ...
 * \param output
 *    Image dimensions
 * \param img_size_x ex:176
 *    coded image horizontal size (luma)
 * \param img_size_y ex:144
...
void pad_borders (FrameFormat output, int img_size_x, int img_size_y, int img_size_x_cr, int img_size_y_cr, imgpel **pImage[3])
{
  ...
  // padding right border   ex:output.width ={176, 88, 88}  output.height ={144, 72, 72} 88=176/2 此值都是像素单位
