...
typedef struct
{
  ...
  Boolean   transform_8x8_mode_flag;     //u(1)等于1表示8x8变换解码过程可能正在使用（参见8.5 节）等于(默认)表示未使用8x8变换解码过程
  ...
  unsigned int num_slice_groups_minus1;                          // ue(v) 加1后指明图像中片组个数
...
typedef struct
{
  ...
  unsigned int pic_order_cnt_type;  //POC mode
