...
{  
  ...
  int         width[3];                      //!< component frame width  Y/U/V  {176, 88, 88}  
  int         height[3];                     //!< component frame height            {144, 72, 72}
  ...
  int         pic_unit_size_on_disk;         //!< picture sample unit size on storage medium一个像素由多少个位表示
  int         pic_unit_size_shift3;          //!< pic_unit_size_on_disk >> 3  |一个像素由多少个字节表示|字节单位大小divide 8 to byte format
} FrameFormat;
