...
static void read_CBP_and_coeffs_from_NAL_CABAC_420(Macroblock *currMB)
{
  ...
  //如果当前宏块不是帧内16*16或者I_PCM类型宏块则从码流中读取CBP
  if (!IS_I16MB (currMB))
  {
    int need_transform_size_flag;
    //=====   C B P   =====
    //---------------------
    //获取CBP read from the stream
  ...
  else // read DC coeffs for new intra modes 是帧内16*16或者I_PCM类型宏块
  {
    cbp = currMB->cbp;
  ...
  if (cbp)
  {
    ...
    {
      //======= 8x8 transform size & CABAC ========
      currMB->read_comp_coeff_8x8_CABAC (currMB, &currSE, PLANE_Y); //read_comp_coeff_8x8_MB_CABAC
    }
    else
    {
