...
************************************************************************
* \brief
*    fill IO buffer 读annex_b->iIOBufferSize到iobuffer进行解码操作
*   return: 实际读到的字节数
************************************************************************
*/
...
{
  if (0 == annex_b->bytesinbuffer)  //如果还未读码流,则读 在open_annex_b读过一次
  {
    if (0 == getChunk(annex_b))	//读一块数据大小为annex_b->iIOBufferSize
      return 0;
  }
