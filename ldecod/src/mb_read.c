...
static void read_ipred_modes(Macroblock *currMB)
{
  ...
  //读取prev_intra4x4_pred_mode_flag/prev_intra8x8_pred_mode_flag
  ...
  if ((dec_picture->chroma_format_idc != YUV400) && (dec_picture->chroma_format_idc != YUV444))
  {
    ...
    if (p_Vid->active_pps->entropy_coding_mode_flag == (Boolean) CAVLC || dP->bitstream->ei_flag) 
      currSE.mapping = linfo_ue;	//ue(v)
    else
      currSE.reading = readCIPredMode_CABAC;	//ae(v)
 ...
 * \brief
 *    init macroblock
 * 将亮度块中的16个4*4块的预测模式设置为2(直接预测),运动向量置0
 ************************************************************************
 */
...
/****宏块层预测语法************************************************************/   
  ...
  // read CBP and Coeffs  ***************************************************************
  //read_CBP_and_coeffs_from_NAL_CAVLC_420  read_CBP_and_coeffs_from_NAL_CABAC_420
/****残差语法************************************************************/  
...
static void read_one_macroblock_i_slice_cavlc(Macroblock *currMB)
{
  ...
  update_qp(currMB, currSlice->qp);	//设置当前宏块的QP,残差系数读取函数
  ...
  // read MB aff
  if (currSlice->mb_aff_frame_flag && (mb_nr&0x01)==0)	//mb_nr&0x01 == mb_nr%2
  {
    TRACE_STRING("mb_field_decoding_flag");
    currSE.len = (int64) 1;
    readSyntaxElement_FLC(&currSE, dP->bitstream);	//u(1)
    currMB->mb_field = (Boolean) currSE.value1;		//0~1
  }

/****宏块层语法************************************/
  //read MB type 
  TRACE_STRING("mb_type");
  //readSyntaxElement_UVLC readSyntaxElement_CABAC
  dP->readSyntaxElement(currMB, &currSE, dP);	//CAVLC:ue(v)|CABAC:ae(v)
  ...
  //模式解释
  //interpret_mb_mode_I interpret_mb_mode_P  interpret_mb_mode_B
