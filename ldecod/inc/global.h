...
typedef struct macroblock_dec
{
  ...
  int                 mbAddrX;                    //!< current MB address (ex: 0~98)
  int mbAddrA, mbAddrB, mbAddrC, mbAddrD;  //左,上,右上,左上
  ...
  Boolean       is_v_block;
  Boolean       DeblockCall;  //当前图像是否正在做去块滤波

  short         slice_nr;  //slice number
  ...
  char          b8pdir[4];
  char          ipmode_DPCM;
  char          c_ipred_mode;       //!< 色度帧内预测模式chroma intra prediction mode
  char          skip_flag;
  short         DFDisableIdc;
  short         DFAlphaC0Offset;
  short         DFBetaOffset;

  Boolean       mb_field;	//在帧场自适应中指明 0帧模式 1场模式
  ...
  Boolean       luma_transform_size_8x8_flag; //对应于transform_size_8x8_flag 
...
{
  int           type;                  //!< SE_type type of syntax element for data part.
...
typedef struct slice
{
  ...
  int                 mb_aff_frame_flag;	// 1:帧场自适应
  int                 direct_spatial_mv_pred_flag;	//!< Indicator for direct mode type (1 for Spatial空间预测, 0 for Temporal时间预测)
  int                 num_ref_idx_active[2];	//!< 可用参考帧的数目number of available list references
...
#endif
  int                 layer_id;
  short               DFDisableIdc;     //!< disable_deblocking_filter_idc Disable deblocking filter on slice
  short               DFAlphaC0Offset;  //!< slice_alpha_c0_offset_div2 Alpha and C0 offset for filtering slice
  short               DFBetaOffset;     //!< slice_beta_offset_div2 Beta offset for filtering slice

  int                 pic_parameter_set_id;   //!<the ID of the picture parameter set the slice is reffering to片需要引用的PPS
  ...
  int  (*nal_startcode_follows    )    (struct slice*, int );	//CAVLC:uvlc_startcode_follows CABAC:cabac_startcode_follows
...
typedef struct video_par
{
  ...
  subset_seq_parameter_set_rbsp_t SubsetSeqParSet[MAXSPS]; //seq_parameter_set_extension_rbsp
  ...
  //int sp_switch;                              //!< 1 for switching sp, 0 for normal sp  
  int type;                  //!< SliceType image type INTER/INTRA  SI_SLICE/I_SLICE etc
  ...
  int newframe;
  int structure;                     //!< PictureStructure Identify picture structure type
  ...
  unsigned int PicHeightInMbs;	//计算在RestOfSliceHeader()中
  unsigned int PicSizeInMbs;	//计算在RestOfSliceHeader()中
...
/******************* deprecative variables; ***************************************/
  ...
  unsigned int PicHeightInMapUnits;
  unsigned int FrameHeightInMbs;
  unsigned int FrameSizeInMbs;	//计算在RestOfSliceHeader()中(末尾)
  unsigned int oldFrameSizeInMbs;
  int max_vmv_r;                             //!< maximum vertical motion vector range in luma quarter frame pixel units for the current level_idc
  //int max_mb_vmv_r;                        //!< maximum vertical motion vector range in luma quarter pixel units for the current level_idc
...
int ExtractionOn;
int ExtractionDisableScreen;
int ExtractionPrint;
int ExtractionCent;
char ExtractionLogFile[FILE_NAME_SIZE];
